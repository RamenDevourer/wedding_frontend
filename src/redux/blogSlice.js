...
export const blogApiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getAllTags: builder.query({
      query: (params = {}) => {
        const queryParams = new URLSearchParams();
        const { user, sort } = params;

        // Add sort parameter if available
        if (sort) queryParams.append("sort", sort);

        // For admin users, use the /allTags endpoint that returns all tags without pagination
        if (
        ...
        // For public users, use /tags but with a large limit
        queryParams.append("t", "1000");
    ...
    deleteTag: builder.mutation({
      query: (body) => ({ url: `/tags`, method: "DELETE", body }),
    }),
    ...
    toggleLikeBlog: builder.mutation({
      query: (blogId) => ({ url: `/${blogId}/like`, method: "POST" }),
    }),
